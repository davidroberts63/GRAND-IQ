...
Properties {
    ...
    $ModuleName = (Get-Item $PSScriptRoot\source\*.psd1)[0].BaseName
...
Task Build -depends Clean -requiredVariables PublishDir, Exclude, ModuleName {
    Copy-Item $PSScriptRoot\Source\* -Destination $PublishDir -Recurse -Exclude $Exclude
...
Task StoreKey -requiredVariables EncryptedApiKeyPath {
    ...
    Get-NuGetApiKey $NuGetApiKey $EncryptedApiKeyPath | Out-Null
